...
@pytest.mark.parametrize('ncols', [unit_param(16)])
@pytest.mark.parametrize('n_info', [unit_param(7)])
def test_rf_classification_pickle(tmpdir, datatype, model, nrows, ncols, n_info):
    result = {}
    def create_mod():
...
def test_decomposition_pickle(tmpdir, datatype, model, data_size):
    ...
    def assert_model(pickled_model, X_test):
        assert array_equal(result["decomposition"], pickled_model.transform(X_test))
...
def test_umap_pickle(tmpdir, datatype, model):
    ...
    def assert_model(pickled_model, X_train):
        ...
        assert array_equal(result["umap_embedding"][0][0], cu_after_embed[0][0])

        cu_trust_after = trustworthiness(X_train, pickled_model.transform(X_train), 10)
