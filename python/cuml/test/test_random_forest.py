...
def test_rf_regression_seed(datatype, column_info, nrows):
    ...
    for i in range(20):
        ...
        cu_reg3 = curfr(seed=seed, n_streams=1)
        cu_reg3.fit(X_train, y_train)

        fil_preds_run3 = cu_reg3.predict(X_test,
                                         predict_model="GPU").copy_to_host()
        fil_r2_3 = r2_score(y_test, fil_preds_run3, convert_dtype=datatype)

        assert array_equal(fil_r2_orig, fil_r2_rerun, tol=1e-4)
        assert array_equal(fil_r2_3, fil_r2_rerun, tol=1e-4)
        assert array_equal(fil_preds_orig, fil_preds_rerun, tol=1e-4)
        assert array_equal(fil_preds_run3, fil_preds_rerun, tol=1e-4)
