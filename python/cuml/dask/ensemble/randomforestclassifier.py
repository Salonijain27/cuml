...
class RandomForestClassifier:
    ...
    @staticmethod
    def _convert_to_tl(model, model_ptr, worker_numb, model_bytes):
        return model._convert_to_treelite(ptr_val=model_ptr,
                                          task_category=1,
                                          worker=worker_numb,
                                          model_pbuf_bytes=model_bytes)

    def convert_to_treelite_mnmg(self):
        """
        ...
        for w in self.workers:
            ...
            print("worker_numb in dask: ", worker_numb)
            print("w[1] in dask : ", w)
            ## or just call the convert treelite function without the c.submit
            var = c.submit(
            ...
            wait(var)
            temp = var.result()
            print(" the model pointer value in MNMG : ", var.result())
            futures.append(var)
            print("length of future in the for loop : ", np.shape(futures))
    ...
    def predict(self, X):
        ...
        concat_conv_tree = self.convert_to_treelite_mnmg()
        ...
        for i in range(len(X)):
            ...
            max_class = -1
            max_val = 0
            #print("len of rslts: ", len(rslts))
            for d in range(len(rslts)):
                #print("self.n_estimators_per_worker[d]: ", self.n_estimators_per_worker[d])
                for j in range(self.n_estimators_per_worker[d]):
                    ...
                    ### Voting on the predicted class label
                    cls = rslts[d][sub_ind]
                    ...
                    if classes[cls] > max_val:
                        max_val = classes[cls]
                        max_class = cls
                #print("val of indexes[d]: ", indexes[d])
                indexes[d] = indexes[d] + self.n_estimators_per_worker[d]
                #print("new val of indexes[d] after update: ", indexes[d])
            #print("value of i: ", i)
